...
const FEATURES: { icon: string; title: string; sub: string; free?: boolean }[] = [
  { icon: '📦', title: 'Add items with photos', sub: 'Snap it, name it, note what it cost.', free: true },
  { icon: '📄', title: 'Insurance-ready PDF report', sub: 'The document your insurer asks for after a loss.' },
  { icon: '♾️', title: 'Unlimited items', sub: 'Free documents 30 items. Pro covers your whole home.' },
...
export default function PaywallModal({ privacyUrl }: { privacyUrl: string }) {
  ...
  const buy = async () => {
    ...
    if (res.ok && res.isPro) {
      setIsPro(true);
      hidePaywall();
      Alert.alert('You’re all set', 'Your whole home is covered, no limits.');
      ...
      <View style={[styles.container, { backgroundColor: theme.bg }]}>
        <ScrollView contentContainerStyle={styles.scroll}>
          ...
          <Text style={[styles.subtitle, { color: theme.textSecondary }]}>
            Start with {weeklyTrial ?? '3 days free'}. Ten minutes of documenting today can be worth thousands on the day you file a claim.
          </Text>
